...
from gaussian_renderer import render_hs, render
import sys
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, args):
    ...
    gaussians = HorseshoeModel(dataset.sh_degree, opt.optimizer_type, args.beta_rho_scale)
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        with torch.no_grad():
            ...
            # Log and save
            training_report(dataset, tb_writer, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end), testing_iterations, scene, render_hs, (pipe, background), args.sample_n, before_selection)
...
def training_report(dataset, tb_writer, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs, sample_n, before_selection=False):
    if tb_writer:
