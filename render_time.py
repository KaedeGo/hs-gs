...
def render_set(dataset, name, iteration, train_views, test_views, gaussians, pipeline, background, depth_scale):
    ...
    times = []
    print(f"The length of train_views: {len(train_views)}")
    print(f"The length of test_views: {len(test_views)}")
    ...
    print(f"The length of times: {len(times)}")
    print(f"Average time taken: {times.mean()} seconds")
    print(f"Standard deviation: {times.std()} seconds")
    # psnr_all /= len(views)
...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams):
    with torch.no_grad():
        ...
        render_set(dataset, "test", scene.loaded_iter, scene.getTrainCameras(), scene.getTestCameras(), gaussians, pipeline, background, scene.depth_scale)
...
if __name__ == "__main__":
    ...
    '''
    The length of times: 1000 [18/09 21:30:33]
    Average time taken: 0.035239163875579835 seconds [18/09 21:30:33]
    Standard deviation: 0.005492422546540761 seconds [18/09 21:30:33]
    '''

    # python render_time.py -m "output/best/LF/torch"