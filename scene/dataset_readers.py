...
class CameraInfo:
    def __init__(self, uid, R, T, FovY, FovX, image, image_path, image_name, width, height, depth=None):
        self.uid = uid
        self.R = R
        self.T = T
        self.FovY = FovY
...
class CameraInfoOld(NamedTuple):
    R: np.array
    T: np.array
    FovY: np.array
    FovX: np.array
...
def readLFSceneInfo(path, images, eval, llffhold=8):
